...
exports.updateMe = catchAsync(async (req, res, next) => {
  ...
  // 3) Check if new username or email already exists
  ...
  if (existingEmailUser && existingUsernameUser) {
    return next(
      new AppError('Both the new email and username are already taken.', 400)
    );
  }

  if (existingEmailUser) {
    return next(new AppError('The email is already taken.', 400));
  }
